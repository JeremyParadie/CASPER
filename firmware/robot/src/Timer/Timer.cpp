...
Timer::Timer(byte timerNr, tc_counter_size countersize, byte pin, unsigned period_us, int pulseWidth_us, bool timerEnable): 
...
Timer::Timer(byte timerNr, void (*_ISR)(), unsigned period_us, bool ISRenable):
...
void Timer::setPulseWidth(unsigned pulseWidth_us)
{ calc(period_us, pulseWidth_us);
  setPeriodMatch(periodCounter, PWcounter, 1); 
}

void Timer::attachInterrupt(void (*_ISR)(), bool interruptEnable)
{ ISR = _ISR;
  setCallback(interruptEnable, TC_CALLBACK_CC_CHANNEL0, ISR); 
}

void Timer::enableTimer(bool timerEnable)
{ enable(timerEnable);
}

void Timer::enableInterrupt(bool interruptEnable)
{ setCallback(interruptEnable, TC_CALLBACK_CC_CHANNEL0, ISR); 
}
...
void Timer::calc(unsigned period_us, unsigned pulseWidth_us)
