...
void AudioWrapper::initialize(){//Call this once before you use any of the other functions below. Configures audio settings.
  if ((VS1053b.begin()&&SD.begin(ACCS))==true) { // initialise the music player
    VS1053b.setVolume(20,20);  //100 is quiet(est), 0 is loudest
  ...
  lastError="Error: AudioWrapper.initialize(); Initialization failure.";//And generate the error
  return;                                       //return
}
...
String AudioWrapper::getVolume(){return "Not implemented yet";}
