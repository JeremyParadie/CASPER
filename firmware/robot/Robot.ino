...
void parse(String command){
  ...
  if(firstWordOf(command,"Servo")){//if the first word of command is "Servo"
    ...
    if(firstWordOf(command,"0")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(0));return;}
    }
    if(firstWordOf(command,"1")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(1));return;}
    }
    if(firstWordOf(command,"2")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(2));return;}
    }
    if(firstWordOf(command,"3")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(3));return;}
    }
    if(firstWordOf(command,"4")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(4));return;}
    }
    if(firstWordOf(command,"5")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(5));return;}
    }
    if(firstWordOf(command,"6")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(6));return;}
    }
    if(firstWordOf(command,"7")){
      ...
      if(firstWordOf(command,"goLimp"))            {Radio.send(Servo.goLimp(7));return;}
    }
  }
  ...
  command.remove(0,String("Display ").length());//remove the first word of the command
    ...
    if(firstWordOf(command,"show")){
      ...
      if(firstWordOf(command,"all"))               {Radio.send(Display.showAll());return;}
      if(firstWordOf(command,"title"))             {Radio.send(Display.showTitle());return;}
      if(firstWordOf(command,"loopCount"))         {Radio.send(Display.showLoopCount());return;}
      if(firstWordOf(command,"commandCount"))      {Radio.send(Display.showCommandCount());return;}
      if(firstWordOf(command,"totalErrorCount"))   {Radio.send(Display.showTotalErrorCount());return;}
      if(firstWordOf(command,"line0"))             {Radio.send(Display.showLine0());return;}
      if(firstWordOf(command,"line1"))             {Radio.send(Display.showLine1());return;}
      if(firstWordOf(command,"line2"))             {Radio.send(Display.showLine2());return;}
      if(firstWordOf(command,"line3"))             {Radio.send(Display.showLine3());return;}
      if(firstWordOf(command,"signal"))            {Radio.send(Display.showSignal());return;}
      if(firstWordOf(command,"battery"))           {Radio.send(Display.showBattery());return;}
      if(firstWordOf(command,"volume"))            {Radio.send(Display.showVolume());return;}
      if(firstWordOf(command,"timeout"))           {Radio.send(Display.showAll());return;}
    }
    if(firstWordOf(command,"hide")){
      command.remove(0,String("hide ").length());//remove the first word of the command
      if(firstWordOf(command,"all"))               {Radio.send(Display.hideAll());return;}
      if(firstWordOf(command,"title"))             {Radio.send(Display.hideTitle());return;}
      if(firstWordOf(command,"loopCount"))         {Radio.send(Display.hideLoopCount());return;}
      if(firstWordOf(command,"commandCount"))      {Radio.send(Display.hideCommandCount());return;}
      if(firstWordOf(command,"totalErrorCount"))   {Radio.send(Display.hideTotalErrorCount());return;}
      if(firstWordOf(command,"line0"))             {Radio.send(Display.hideLine0());return;}
      if(firstWordOf(command,"line1"))             {Radio.send(Display.hideLine1());return;}
      if(firstWordOf(command,"line2"))             {Radio.send(Display.hideLine2());return;}
      if(firstWordOf(command,"line3"))             {Radio.send(Display.hideLine3());return;}
      if(firstWordOf(command,"signal"))            {Radio.send(Display.hideSignal());return;}
      if(firstWordOf(command,"battery"))           {Radio.send(Display.hideBattery());return;}
      if(firstWordOf(command,"volume"))            {Radio.send(Display.hideVolume());return;}
      if(firstWordOf(command,"timeout"))           {Radio.send(Display.hideTimeout());return;}      
    }
    if(firstWordOf(command,"isShowing")){
      command.remove(0,String("isShowing ").length());//remove the first word of the command
      if(firstWordOf(command,"title"))             {Radio.send(Display.isShowingTitle());return;}
      if(firstWordOf(command,"loopCount"))         {Radio.send(Display.isShowingLoopCount());return;}
      if(firstWordOf(command,"commandCount"))      {Radio.send(Display.isShowingCommandCount());return;}
      if(firstWordOf(command,"totalErrorCount"))   {Radio.send(Display.isShowingTotalErrorCount());return;}
      if(firstWordOf(command,"line0"))             {Radio.send(Display.isShowingLine0());return;}
      if(firstWordOf(command,"line1"))             {Radio.send(Display.isShowingLine1());return;}
      if(firstWordOf(command,"line2"))             {Radio.send(Display.isShowingLine2());return;}
      if(firstWordOf(command,"line3"))             {Radio.send(Display.isShowingLine3());return;}
      if(firstWordOf(command,"signal"))            {Radio.send(Display.isShowingSignal());return;}
      if(firstWordOf(command,"battery"))           {Radio.send(Display.isShowingBattery());return;}
      if(firstWordOf(command,"volume"))            {Radio.send(Display.isShowingVolume());return;}
      if(firstWordOf(command,"timeout"))           {Radio.send(Display.isShowingTimeout());return;}     
    }
    if(firstWordOf(command,"setText")){
      command.remove(0,String("setText ").length());//remove the first word of the command}
      if(firstWordOf(command,"line0"))             {command.remove(0,String("line0 ").length());Radio.send(Display.setTextLine0(command));return;}
      if(firstWordOf(command,"line1"))             {command.remove(0,String("line1 ").length());Radio.send(Display.setTextLine1(command));return;}
      if(firstWordOf(command,"line2"))             {command.remove(0,String("line2 ").length());Radio.send(Display.setTextLine2(command));return;}
      if(firstWordOf(command,"line3"))             {command.remove(0,String("line3 ").length());Radio.send(Display.setTextLine3(command));return;} 
    }
    if(firstWordOf(command,"getText")){
      command.remove(0,String("getText ").length());//remove the first word of the command}
      if(firstWordOf(command,"line0"))             {Radio.send(Display.getTextLine0());return;}
      if(firstWordOf(command,"line1"))             {Radio.send(Display.getTextLine1());return;}
      if(firstWordOf(command,"line2"))             {Radio.send(Display.getTextLine2());return;}
      if(firstWordOf(command,"line3"))             {Radio.send(Display.getTextLine3());return;} 
    }
  }
  if(firstWordOf(command,"Battery")){//if the first word of command is "Battery"
    ...
    if(firstWordOf(command,"setMaxCapacity"))      {command.remove(0,String("setMaxCapacity ").length());Radio.send(Battery.setMaxCapacity(command));return;}
    if(firstWordOf(command,"setMinimum"))          {command.remove(0,String("setMinimum ").length());Radio.send(Battery.setMinimum(command));return;}
  }
  ...
  if(firstWordOf(command,"FRAM")){//if the first word of command is "FRAM"
	...
	if(firstWordOf(command, "getData")){command.remove(0, String("getData ").length());Radio.send(Fram.getData(command));return;}
  ...
  if(firstWordOf(command,"System")){//if the first word of command is "System"
   ...
   # if(firstWordOf(command,"goToSleep"))           {Radio.send(goToSleep());return;}
    ...
    if(firstWordOf(command,"getAvailableMemory"))  {Radio.send(String(freeMemory())+" bytes");return;}
    if(firstWordOf(command,"getVersion"))          {Radio.send(VERSION);return;}
    if(firstWordOf(command,"ping"))                {Radio.send("pong");return;}
    if(firstWordOf(command,"getTimeLeft"))         {Radio.send(getTimeLeft());return;}
	if(firstWordOf(command,"forceCrash"))          {Radio.send(forceCrash());return;}
